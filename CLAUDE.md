...
- Package manager is **pnpm**. CI installs with `pnpm ci` — a real pnpm 11 command, not a typo for `npm ci`; do not "fix" it. Both workflows resolve Node from `.node-version`, which holds the major (`24`) rather than an exact version or `lts/*` — patch updates land automatically, and moving to the next LTS line (Node 26 is promoted 2026-10-28) is a deliberate one-line PR. Keep it aligned with the `engines` floor. The Release job was temporarily pinned to `24.16.0` because Node 24.17.0's `http.Agent` fix (CVE-2026-48931) made `node-fetch@2` — used by `@changesets/get-github-info` — throw a false `ERR_STREAM_PREMATURE_CLOSE` during `changeset version`; [nodejs/node#64004](https://github.com/nodejs/node/pull/64004) fixed that in 24.18.0, so the pin is gone. If a "Premature close" failure ever reappears in Release, that regression is the thing to check.
